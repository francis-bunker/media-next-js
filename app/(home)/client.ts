...
export const getPostsByPlaceId = async (placeId: string) => {
    const response = await axios.get(`/api/posts/place/${placeId}`);
    return response.data;
}
...
export const searchPlaces = async (query: string) => {
    const response = await axios.post(`/api/map/search`, {
        query
    });
    return response.data;
};
...
export const getPlaceTitle = async (placeId: string) => {
    const response = await axios.get(`/api/map/place/${placeId}/title`);
    return response.data;
}
